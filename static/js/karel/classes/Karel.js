(function($) {
	KarelApp.Karel = Romano.Sprite.extend({
		init: function(viewport, appInstance) {
			...
			this.body.getRenderer().setSource('/static/js/karel/images/gir-body.svg');
			this.head = new Romano.Sprite('gir-head', viewport, new Romano.RaphaelRenderer(), {
				position: { x: -20, y: -60 },
				rotationCenter: { x: 25, y: 50 }
			});
			this.head.getRenderer().setSource('/static/js/karel/images/gir-head.svg');
			this.leftJet = new Romano.Sprite('gir-left-jet', viewport, new Romano.RaphaelRenderer(), {
				position: { x: -20, y: 56 },
				rotationCenter: { x: 0, y: 0 }
			});
			this.leftJet.getRenderer().setSource('/static/js/karel/images/flame-1.svg', function(sprite) {
			...
			this.rightJet.getRenderer().setSource('/static/js/karel/images/flame-1.svg', function(sprite) {
				sprite.setRotationCenter(0, 0);
				sprite.setScale(1, 0);
			});
