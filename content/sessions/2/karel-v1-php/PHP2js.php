...
class PHP2js {
	...
	public function __construct ($file) {
		...
		$this->current_char = 0;
		// Hack: originally dies() when done. (Which is horrible.) 
		// Now we just catch an exception and return, which is also horrible.
		try {
			$this->compileJs();
		}
		catch (Exception $e) {
	...
	private function parseToken ($name, $value, & $js, $_schema=array()) {
		...
		if ($name == 'T_STRING' && $value == 'error_log') {
	...
	private function T_FOR($value) {
		while (true) {
			$this->next($name, $value);
			if ($name == '(') {
				$init = $this->parseUntil(array(';'));
				$condition = $this->parseUntil(array(';'));
				$step = $this->parseUntil(array(')'));
				$this->js .= "for ($init; $condition; $step) ";
				return '';
			}
		}
	}
	...
	private function handle_error_log() {
		...
		while (true) {
			...
			if ($name == ')') {
				$parens--;
				$endIndex = $this->current_char;
			}
			if ($started && $parens == 0) {
				//error_log($startIndex.', '.$endIndex);
				eval('error_log('.substr($this->src, $startIndex + 6, $endIndex - $startIndex - 1).');');
				return;
			}
		}
	}
	...
	private function getToken(& $name, & $value, $_token) {
		if (is_array($_token)) {
			...
			$value = $_token[1];
			// error_log($_token[1].', '.strlen($_token[1]));
			$this->current_char += strlen($_token[1]);
		} else {
			$name = trim($_token);
			$value = '';
			// error_log($_token.', '.strlen($_token));
			$this->current_char += strlen($_token);
		}
	}
	...
	private $_openTags = array(
			'T_OPEN_TAG', 'T_CLASS', 'T_PUBLIC', 'T_FOREACH', 'T_FOR', 'T_ARRAY', '{', 'T_VARIABLE', '('
			);
