...
<body>
	...
	<script type="text/javascript">
		...
		function loadPage(pageNum) {
			var startEntryNum = pageNum * gEntriesPerPage;
			var lastEntryNum = startEntryNum + gEntriesPerPage;

			$('#entries').empty();

			for (var i = startEntryNum; (i < lastEntryNum && i < gEntries.length); i++) {
				var markup = '\
					<li class="row">\
						<div class="3u">\
							<h3>' + gEntries[i].title + '</h3>\
							<div class="byline">' + gEntries[i].name + '</div>\
							<div class="dateline">' + gEntries[i].timestamp + '</div>\
							<div class="email">' + gEntries[i].email + '</div>\
						</div>\
						<div class="9u">\
							' + gEntries[i].message + '\
						</div>\
		...
		$(document).ready(function() {
			...
			for (var i = 0; i < nPages; i++) {
				...
				$('nav#pagination ul').append('<li><a data-pagenum="' + i + '" href="#">' + (i + 1) + '</a></li>');
			}
			// now, attach a handler to each of those nav <li>s.
			for (i = 0; i < nPages; i++) {
				$('nav#pagination li a').eq(i).on('click', function() {
