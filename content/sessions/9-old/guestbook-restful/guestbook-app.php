...
define('ROOT_URL', 'http://intro-web-programming.dev/content/sessions/9/guestbook-restful/');
define('GUESTBOOK_ROOT', '/content/sessions/9/guestbook-restful/');
...
function printPaginationLinks() {
	...
	for ($i = 0; $i < $nPages; $i++) {
		echo '<li><a '.($currentPage == $i ? 'class="active"' : '').'href="'.ROOT_URL.'page/'.($i + 1).'">'.($i + 1).'</a></li>';
	}
}