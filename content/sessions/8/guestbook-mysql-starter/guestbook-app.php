...
function addEntryToGuestbook($newEntry) {
	...
	if (empty($gValidationErrors)) {
		...
		// !!!! We no longer need messages.json. Instead, we're
		// going to insert the sanitized, validated data into the
		// database.
...
function printJSONEntries() {
	...
	// !!!! Here, rather than getting data from a file we'll be
	// querying the database. The raw query result must be converted
	// first into a PHP associative array with the format expected
	// by our (currently Javascript-powered) template code (see
	// the loadPage function in index.php). Then, the PHP associative
	// array must be converted into JSON.
