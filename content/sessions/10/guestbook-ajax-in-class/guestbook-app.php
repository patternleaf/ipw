...
define('ROOT_URL', 'http://intro-web-programming.dev/content/sessions/10/guestbook-ajax-in-class/');
define('GUESTBOOK_ROOT', '/content/sessions/10/guestbook-ajax-in-class/');
...
function printPaginationLinks() {
	...
	for ($i = 0; $i < $nPages; $i++) {
		echo '
			<li>
			<a data-pagenum="'.($i + 1).'" '.($currentPage == $i ? 'class="active"' : '').'href="'.ROOT_URL.'page/'.($i + 1).'">'.($i + 1).'</a>
			</li>
		';
	}
}